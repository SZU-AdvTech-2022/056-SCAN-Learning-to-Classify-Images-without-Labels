...
class ImageNetSubset(data.Dataset):
    def __init__(self, subset_file, root=MyPath.db_root_dir('imagenet'), split='train',
        ...
        self.root = os.path.join(root, 'imagenet_dog/%s' %(split))
        ...
        imgs = []
        targets = []
        if 'imagenet_tiny' in subset_file and split == 'train':
            self.root = '/public/dataset/tiny-imagenet-200/train'
            for i, subdir in enumerate(subdirs):
                files = sorted(glob(os.path.join(self.root, subdir, 'images','*.JPEG')))
                for f in files:
                    imgs.append((f, i))
                    targets.append(i)
        else:
