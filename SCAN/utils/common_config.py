...
def get_train_dataset(p, transform, flag, to_augmented_dataset=False, to_neighbors_dataset=False, split=None, debug=False):
    # Base dataset
    if p['train_db_name'] == 'cifar-10':
        from data.cifar import CIFAR10
        dataset = CIFAR10(train=True, transform=transform, download=True, debug=debug)
        print('1')
    ...
    if to_neighbors_dataset: # Dataset returns an image and one of its nearest neighbors.
        ...
        indices = np.load(p['top{}_neighbors_train_path'.format(p['num_neighbors'])]) # 算好了近邻，这里算是全图
        if debug:
        ...
        # count truly knn samples
        # targets = dataset.targets
        # targets = torch.tensor(targets)
        # knn_targets = targets[indices[:, 1:]]
        # targets = targets.view(-1, 1).expand_as(knn_targets)
        # eq = torch.sum(targets == knn_targets, dim=1)
        # torch.sum(eq) / eq.shape[0]

    return dataset


def get_val_dataset(p, args, transform=None, to_neighbors_dataset=False):
    # Base dataset
    ...
    if to_neighbors_dataset: # Dataset returns an image and one of its nearest neighbors.
        ...
        dataset = NeighborsDataset(dataset, indices, args.R, args.minibatch, 5) # Only use 5
...
def get_train_dataloader_nosuf(p, dataset):
    return torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'],
            batch_size=p['batch_size'], pin_memory=True, collate_fn=collate_custom,
...
def get_train_transformations(p):
    if p['augmentation_strategy'] == 'standard':
    ...
    elif p['augmentation_strategy'] == 'cutmix':
        # Augmentation strategy from the CutMix paper
        ...
        return transforms.Compose([
            ...
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            transforms.Normalize(**p['augmentation_kwargs']['normalize']) # normalize,
        ])
...
def get_weak_transformations(p):
    return transforms.Compose([
        transforms.RandomHorizontalFlip(),
        transforms.RandomCrop(p['augmentation_kwargs']['crop_size'], padding=4,padding_mode="reflect"),
        Augment(1),
        transforms.ToTensor(),
        transforms.Normalize(**p['augmentation_kwargs']['normalize'])
    ])
