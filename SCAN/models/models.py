...
class ClusteringModel(nn.Module):
    def __init__(self, backbone, nclusters, nheads=1):
        ...
        self.cluster_head = nn.ModuleList([self.cluster_projector for _ in range(self.nheads)])
"""
...
class ClusteringModel(nn.Module):
    ...
    def forward(self, x, forward_pass='default'):
        if forward_pass == 'default':
            ...
            features = self.backbone(x)
            out = {'features': features, 'output': [cluster_head(features) for cluster_head in self.cluster_head]} # 这里的线性层，将特征映射为10类
...
# class ClusteringModel(nn.Module):
#     def __init__(self,  nclusters, nheads=1):
#         super(ClusteringModel, self).__init__()
#
#         self.nheads = nheads
#         self.cluster_projector = nn.Sequential(
...
#
#         predictions = self.cluster_projector(features)
#
#         return predictions
